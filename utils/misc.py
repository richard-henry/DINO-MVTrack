...
def posemb_sincos_3d_xyz(xyz, C, temperature=10000, dtype=torch.float32, cat_coords=False):
    device = xyz.device
    dtype = xyz.dtype
    B, S, D = xyz.shape
    assert(D==3)
    ...
    y = xyz[:,:,1] 
    z = xyz[:,:,2]
    assert (C % 6) == 0, 'feature dimension must be multiple of 6 for sincos emb'
    omega = torch.arange(C // 6, device=device) / (C // 6 - 1)
    omega = 1. / (temperature ** omega)
    ...
    pe = pe.reshape(B,S,C).type(dtype)
    if cat_coords:
        pe = torch.cat([pe, xyz], dim=2) # B,N,C+3
    return pe
